...
"Fresh" (w == 1) mode
---------------------
With no previous action chunk, ``sample_actions`` should reduce to vanilla pi05
sampling (weight ``w == 1`` everywhere). NOTE: passing nothing does *not* achieve
this -- ``warmup`` is forced to 0 but ``ramp`` defaults to ``config.ramp_min``,
...
``warmup=0, ramp=1`` provably yields ``w == 1`` for every position. This script
therefore drives the fresh path with ``warmup=0, ramp=1`` by default. See
``--ramp`` / ``--warmup`` to override.
...
-------
    python scripts/validate_sample_actions.py \
        --policy-path /path/to/outputs/.../checkpoints/050000/pretrained_model \
...
from lerobot_policy_pi05_legato.modeling_pi05_legato import PI05LegatoPolicy  # noqa: E402
...
def evaluate(
    ...
    Returns a dict with overall and per-episode MSE, in both the model's
    (normalized) action space and the raw/absolute space.
    """
    ...
    kwargs = {"warmup": warmup, "ramp": ramp}
    ...
    for bi, batch in enumerate(loader):
        ...
        # Fully-denoised predicted chunk (w == 1 when warmup=0, ramp=1).
        pred = policy.predict_action_chunk(batch, **kwargs)  # (B, T, Da), model space
        target_norm = batch[ACTION][..., :original_action_dim].to(pred.device).float()  # (B, T, Da)

        valid = (~pad).to(pred.device).unsqueeze(-1).float()  # (B, T, 1)
...
def main() -> None:
    ...
    parser.add_argument("--warmup", type=int, default=0, help="LEGATO warmup (forced to 0 without a prev chunk). Default 0.")
    parser.add_argument("--ramp", type=int, default=1, help="LEGATO ramp. Default 1 => w==1 everywhere (true vanilla / fully-fresh).")
    parser.add_argument("--compare-base", action="store_true", help="Also evaluate the un-finetuned lerobot/pi05_base baseline.")
    args = parser.parse_args()

    init_logging()

    if args.ramp != 1 or args.warmup != 0:
        logging.warning(
            f"warmup={args.warmup}, ramp={args.ramp}: NOT the fully-fresh (w==1) path. "
            "Without a previous chunk the anchor is zeros, so any w<1 position is pulled toward 0."
        )
    ...
    logging.info("Evaluating finetuned checkpoint sequentially over episodes...")
    ft_result = evaluate(
        finetuned, dataset, dataset_stats, args.device,
        args.batch_size, args.num_workers, args.warmup, args.ramp,
    )
    ...
    if args.compare_base:
        ...
        logging.info("Evaluating pi05_base baseline sequentially over episodes...")
